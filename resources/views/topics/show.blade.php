...
@section('content')
    <div class="container">
        <div class="row align-items-center mb-3">
            ...
            @can('manage', $topic)
                <div class="col-1">
                    <a class="btn btn-sm btn-block btn-outline-info" href="{{ route('topics.edit', ['topic' => $topic]) }}">
        ...
        @foreach ($replies as $reply)
            <div class="card mb-3">
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="col">
                            Posted by <a href="{{ route('users.show', ['user' => $reply->user]) }}">{{ $reply->user->name }}</a>, <time title="{{ $reply->created_at }}">{{ $reply->created_at->diffForHumans() }}</time>.
                        </div>
